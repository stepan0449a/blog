require 'rails_helper'

# This spec was generated by rspec-rails when you ran the scaffold generator.
# It demonstrates how one might use RSpec to test the controller code that
# was generated by Rails when you ran the scaffold generator.
#
# It assumes that the implementation code is generated by the rails scaffold
# generator. If you are using any extension libraries to generate different
# controller code, this generated spec may or may not pass.
#
# It only uses APIs available in rails and/or rspec-rails. There are a number
# of tools you can use to make these specs even more expressive, but we're
# sticking to rails and rspec-rails APIs to keep things simple and stable.

RSpec.describe "/nicknames", type: :request do
  
  # This should return the minimal set of attributes required to create a valid
  # Nickname. As you add validations to Nickname, be sure to
  # adjust the attributes here as well.
  let(:valid_attributes) {
    skip("Add a hash of attributes valid for your model")
  }

  let(:invalid_attributes) {
    skip("Add a hash of attributes invalid for your model")
  }

  describe "GET /index" do
    it "renders a successful response" do
      Nickname.create! valid_attributes
      get nicknames_url
      expect(response).to be_successful
    end
  end

  describe "GET /show" do
    it "renders a successful response" do
      nickname = Nickname.create! valid_attributes
      get nickname_url(nickname)
      expect(response).to be_successful
    end
  end

  describe "GET /new" do
    it "renders a successful response" do
      get new_nickname_url
      expect(response).to be_successful
    end
  end

  describe "GET /edit" do
    it "renders a successful response" do
      nickname = Nickname.create! valid_attributes
      get edit_nickname_url(nickname)
      expect(response).to be_successful
    end
  end

  describe "POST /create" do
    context "with valid parameters" do
      it "creates a new Nickname" do
        expect {
          post nicknames_url, params: { nickname: valid_attributes }
        }.to change(Nickname, :count).by(1)
      end

      it "redirects to the created nickname" do
        post nicknames_url, params: { nickname: valid_attributes }
        expect(response).to redirect_to(nickname_url(Nickname.last))
      end
    end

    context "with invalid parameters" do
      it "does not create a new Nickname" do
        expect {
          post nicknames_url, params: { nickname: invalid_attributes }
        }.to change(Nickname, :count).by(0)
      end

      it "renders a successful response (i.e. to display the 'new' template)" do
        post nicknames_url, params: { nickname: invalid_attributes }
        expect(response).to be_successful
      end
    end
  end

  describe "PATCH /update" do
    context "with valid parameters" do
      let(:new_attributes) {
        skip("Add a hash of attributes valid for your model")
      }

      it "updates the requested nickname" do
        nickname = Nickname.create! valid_attributes
        patch nickname_url(nickname), params: { nickname: new_attributes }
        nickname.reload
        skip("Add assertions for updated state")
      end

      it "redirects to the nickname" do
        nickname = Nickname.create! valid_attributes
        patch nickname_url(nickname), params: { nickname: new_attributes }
        nickname.reload
        expect(response).to redirect_to(nickname_url(nickname))
      end
    end

    context "with invalid parameters" do
      it "renders a successful response (i.e. to display the 'edit' template)" do
        nickname = Nickname.create! valid_attributes
        patch nickname_url(nickname), params: { nickname: invalid_attributes }
        expect(response).to be_successful
      end
    end
  end

  describe "DELETE /destroy" do
    it "destroys the requested nickname" do
      nickname = Nickname.create! valid_attributes
      expect {
        delete nickname_url(nickname)
      }.to change(Nickname, :count).by(-1)
    end

    it "redirects to the nicknames list" do
      nickname = Nickname.create! valid_attributes
      delete nickname_url(nickname)
      expect(response).to redirect_to(nicknames_url)
    end
  end
end
